...
if(isset($_POST['consulta'])){
                                ...
                                $c = new Conexion();
                                $cone = $c->conectando();
                                $sql = "select factura.IdFactura, tipopago.Descripcion_Pago,factura.Fecha, factura.CostEnvio, factura.IvaFactura,factura.ValorFactura from factura inner join tipopago on factura.IdTipoPago = tipopago.IdTipoPago where factura.IdFactura like '%$obj->IdFactura%' ";
                                ...
                                $c = new Conexion();
                                $cone = $c->conectando();
                                $sql = "select factura.IdFactura, tipopago.Descripcion_Pago, factura.Fecha, factura.CostEnvio , factura.IvaFactura, factura.ValorFactura from factura inner join tipopago on factura.IdTipoPago = tipopago.IdTipoPago order by IdFactura";
                                $limite =sprintf("%s limit %d, %d",$sql, $inicia, $maximoDatos);
                                $resultado = mysqli_query($cone,$limite);
                                $arreglo = mysqli_fetch_row($resultado);
...
<html lang="es">
    ...
    <body>
        ...
        <center>
          <br>   
            <h3 class=" text-danger font-weight-normal  " >Factura</h3> 
            <hr style="height:1px;border:none;color:#333;background-color:#333;">
            <form name="Detalle" action="" method="POST">
            <table >
                 <tr>
                    <td>
                             <a  href="FacturaAgregar.php"><button type="button" class="btn btn-primary btn-sm "><i class="fa fa-file-text-o" aria-hidden="true">Agregar</i></button>
                            </a>
                    </td>
                </tr>
                 <tr>
		                <th><label for="proveedor">Buscar</label></th>
		                <th><input style="font-size:12px" type="text" id=" IdFactura" name="IdFactura"   placeholder="Digite el ID de Factura " size="50" >
                        ...
                        <a href="../Reportes/ListadoFactura.php">
            ...
            <center>
                <table class= "table  table-bordered table-hover table-sm"  style="width:90%">
                        <caption><small>Lista de Pago</small></caption>  
                        <thead>
                            <tr class="table-dark">
                                <th scope="col" style="width:10%"  class="text-light letra">IdFactura</th>
                                <th scope="col" style="width:5%"  class="text-light letra">IdTipoPago</th>
                                <th scope="col" style="width:15%" class="text-light letra">Fecha</th>
                                <th scope="col" style="width:5%"  class="text-light letra">CostEnvio</th>
                                <th scope="col" style="width:5%"  class="text-light letra">IvaFactura</th>
                                <th scope="col" style="width:10%"  class="text-light letra">ValorFactura</th>
                        ...
                        <tbody>
                            ...
                            <td class="arreglo2" ><?php echo ($arreglo[1]) ?></td>
                            <td class="arreglo2"><?php echo $arreglo[2] ?>
                            <td class="arreglo2"><?php echo ($arreglo[3]) ?></td>
                            <td class="arreglo2"><?php echo ($arreglo[4]) ?></td>
                            <td class="arreglo2"><?php echo ($arreglo[5]) ?></td>
                                <td class="letra">
                                <a href="<?php if($arreglo[0] <> ""){
                        echo "FacturaModificar.php?key=".urlencode($arreglo[0]);
                        } else{
                         ...
                         ?>">
                            <button name="Guarda" type="button" class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true">Modificar</i></button>
                                ...
                                <td class="letra">
                                <a href="<?php if($arreglo[0]<>""){
                                            echo "FacturaEliminar.php?key=".urlencode($arreglo[0]);
                                    }else{
