...
if(isset($_POST['consulta'])){
                                ...
                                $c = new Conexion();
                                $cone = $c->conectando();
                                $sql = "select * from promocion where  IdPromocion LIKE '%$obj->IdPromocion%'";
                                ...
                                $c = new Conexion();
                                $cone = $c->conectando();
                                $sql = "select * from promocion order by IdPromocion";
                                $limite =sprintf("%s limit %d, %d",$sql, $inicia, $maximoDatos);
                                $resultado = mysqli_query($cone,$limite);
                                $arreglo = mysqli_fetch_row($resultado);
...
<html lang="es">
    ...
    <body>
        ...
        <center>
          <br>   
            <h3 class=" text-danger font-weight-normal  " >Promociones </h3> 
            <hr style="height:1px;border:none;color:#333;background-color:#333;">
            <form name="Detalle" action="" method="POST">
            <table >
                 <tr>
                    <td>
                             <a  href="Prom.php"><button type="button" class="btn btn-primary btn-sm "><i class="fa fa-file-text-o" aria-hidden="true">Agregar</i></button>
                            </a>
                    </td>
                </tr>
                 <tr>
		                <th><label for="promocion">Buscar</label></th>
		                <th><input style="font-size:12px" type="text" id="IdPromocion" name="IdPromocion"   placeholder="Digite el ID de la promocion " size="50" >
            ...
            <center>
                <table class= "table  table-bordered table-hover table-sm"  style="width:90%">
                        <caption><small>Lista de Promociones</small></caption>  
                        <thead>
                            <tr class="table-dark">
                                <th scope="col" style="width:10%"  class="">IdPromocion</th>
                                <th scope="col" style="width:5%"  class="">IdProducto</th>
                                <th scope="col" style="width:10%"  class="">Nombre</th>
                                <th scope="col" style="width:15%" class="">Detalle</th>
                                <th scope="col" style="width:5%"  class="">Fecha_Inicio</th>
                                <th scope="col" style="width:5%"  class="">Fecha_Fin</th>
                                <th scope="col" style="width:5%"  class="">ValorPromocion</th>
                        ...
                        echo "PromModificar.php?key=".urlencode($arreglo[0]);
                        } else{
                         ...
                         ?>">
                            <button name="Guarda" type="button" class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true">Modificar</i></button>
                                ...
                                <td class="letra">
                                <a href="<?php if($arreglo[0]<>""){
                                            echo "PromEliminar.php?key=".urlencode($arreglo[0]);
                                    }else{
