...
if(isset($_POST['consulta'])){
                                ...
                                $c = new Conexion();
                                $cone = $c->conectando();
                                $sql = "select pedidos.IdPedido, pedidos.FechaPedido,pedidos.HoraPedido, cliente.Nombre, pedidos.IvaPedido,pedidos.TotalPedido from pedidos inner join cliente on pedidos.IdCliente = cliente.IdCliente where pedidos.IdPedido like '%$obj->IdPedido%' ";
                                ...
                                $c = new Conexion();
                                $cone = $c->conectando();
                                $sql = "select pedidos.IdPedido, pedidos.FechaPedido, pedidos.HoraPedido, cliente.Nombre, pedidos.IvaPedido, pedidos.TotalPedido from pedidos inner join cliente on pedidos.IdCliente = cliente.IdCliente order by IdPedido";
                                $limite =sprintf("%s limit %d, %d",$sql, $inicia, $maximoDatos);
                                $resultado = mysqli_query($cone,$limite);
                                $arreglo = mysqli_fetch_row($resultado);
        ...
        <center>
          <br>   
            ...
            <table >
                 <tr>
                    <td>
                             <a  href="PedidosAgregar1.php"><button type="button" class="btn btn-primary btn-sm "><i class="fa fa-file-text-o" aria-hidden="true"></i>agregar</button></a>
                    </td>
                </tr>
                 <tr>
		                <th><label for="categorias">Buscar</label></th>
		                <th><input style="font-size:12px" type="text" id="IdPedido" name="IdPedido"   placeholder="Digite  el ID del Pedido " size="50" >
            ...
            <center>
                ...
                <table class="table  table-bordered table-hover table-sm " style="width:90%">
                        <caption><small>Lista de Pedidos</small></caption>  
                        <thead>
                            <tr class="bg-dark">
                                <th scope="col" style="width:10%"  class="text-light letra">IdPedido</th>
                                <th scope="col" style="width:15%" class="text-light letra">FechaPedido</th>
                                <th scope="col" style="width:15%" class="text-light letra">HoraPedido</th>
                                <th scope="col" style="width:15%" class="text-light letra">IdCliente</th>
                                <th scope="col" style="width:15%" class="text-light letra">IvaPedido</th>
                                <th scope="col" style="width:15%" class="text-light letra">TotalPedido</th>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td cass="arreglo"><?php echo ($arreglo[1]) ?></td>
                                <td cass="arreglo"><?php echo ($arreglo[2]) ?></td>
                                <td cass="arreglo"><?php echo ($arreglo[3]) ?></td>
                                <td cass="arreglo"><?php echo ($arreglo[4]) ?></td>
                                <td cass="arreglo"><?php echo ($arreglo[5]) ?></td>
                            
                                <td class="letra">
                                <a href="<?php if($arreglo[0] <> ""){
                        echo "PedidosModificar1.php?key=".urlencode($arreglo[0]);
                        } else{
                            echo "javascript:alert('Debe Consultar El Proveedor');";
                         }
                         ?>">
                                    <button name="modifica" type="button" class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true">Modificar</i></button>
                                    </a> 
                                </td>
                                <td class="letra">
                                <a href="<?php if($arreglo[0] <> ""){
                        echo "CategoriaEliminar.php?key=".urlencode($arreglo[0]);
                        } else{
                            echo "javascript:alert('Debe Consultar El Proveedor');";
                         }
                         ?>">
                                    <button name="elimina" type="button" class="btn btn-danger btn-sm"><i class="fa fa-trash-o" aria-hidden="true">Eliminar</i></button>
                                </a> 
                                </td>
                            </tr>
                        </tbody>
