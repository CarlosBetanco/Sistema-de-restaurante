...
if(isset($_POST['consulta'])){
                                ...
                                $c = new Conexion();
                                $cone = $c->conectando();
                                $sql = "select * from  tipopago  where IdTipoPago LIKE '%$obj->IdTipoPago%'";
                                ...
                                $c = new Conexion();
                                $cone = $c->conectando();
                                $sql = "select * from tipopago order by IdTipoPago";
                                $limite =sprintf("%s limit %d, %d",$sql, $inicia, $maximoDatos);
                                $resultado = mysqli_query($cone,$limite);
                                $arreglo = mysqli_fetch_row($resultado);
...
<html lang="es">
    <head>
        ...
        <title>TipoPago</title>
    </head>
    <body>
        
        <center>
          <br>   
            <h3 class=" text-danger font-weight-normal  " >TipoPago</h3> 
            <hr style="height:1px;border:none;color:#333;background-color:#333;">
            <form name="tipopago" action="" method="POST">
            <table >
                 <tr>
                    <td>
                             <a  href="TipoPagoAgregar.php"><button type="button" class="btn btn-primary btn-sm "><i class="fa fa-file-text-o" aria-hidden="true"></i>agregar</button></a>
                    </td>
                </tr>
                 <tr>
		                <th><label for="tipopago">Buscar</label></th>
		                <th><input style="font-size:12px" type="text" id="IdTipoPago" name="IdTipoPago"   placeholder="Digite  el ID de tipo de pago " size="50" >
            ...
            <center>
                <table class="table  table-bordered table-hover table-sm " style="width:90%">
                        ...
                        <thead>
                            <tr class="bg-dark">
                                <th scope="col" style="width:10%"  class="text-light letra">IdTipoPago</th>
                                <th scope="col" style="width:15%" class="text-light letra">Descripcion_Pago</th>
                        ...
                        echo "TipoPagoModificar.php?key=".urlencode($arreglo[0]);
                        } else{
                         ...
                         ?>">
                                ...
                                <td class="letra">
                                <a href="<?php if($arreglo[0] <> ""){
                        echo "TipoPagoEliminar.php?key=".urlencode($arreglo[0]);
                        } else{
                            echo "javascript:alert('Debe Consultar El Proveedor');";
                         }
                         ?>">
